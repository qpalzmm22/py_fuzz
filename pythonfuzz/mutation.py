...
class Corpus(object):
   ...
   ''' 
    ...
    def generate_input(self, favored):
        found = False
        if not self._seed_run_finished:
            next_input = self._inputs[self._seed_idx]
            self._seed_idx += 1
            #print(str(self._seed_idx) + " " +  str(self._initial_seeds))
            #if self._seed_idx >= self._initial_seeds:
            #    self._seed_run_finished = True
            if self._seed_idx >= len(self._inputs):
                self._seed_run_finished = True
            return next_input

        #self._seed_idx = self._rand(len(self._inputs))
        #buf = self._inputs[self._seed_idx]
        while(!found){
        buf = self._inputs.pop(0)
        # is there an edge using this input as favored?
        # is all branches of this input not favored by this input? (can check with favored..? or link? or count how many favored are there)
        # -> is all favored branch does not have this input?
            for edge in favored:
                if buf == favored[edge] :
                    found = True
                    break;
        ...
        ##  is there non-mutated favored input in the input queue?
        # -> When adding as Favored, we can add them in a set as Not-executed list..?
