...
main(sys.argv, sys.stdin.readline())

def get_line_objs(flags: ScreenFlags,buf) -> Dict[int, LineBase]:
    input_lines = buf.decode("ascii")
...
def do_program(flags: ScreenFlags, buf) -> None:
    file_path = state_files.get_pickle_file_path()
    line_objs = get_line_objs(flags, buf)
...
def main(argv: List[str], buf) -> int:
